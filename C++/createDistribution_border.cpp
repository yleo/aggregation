...
int main(int argc, char *argv[]) {
  ...
  int s1, s2, t1, h,t,ni;
  int maxt=0, maxp=0; 
  double lolo=atoi(argv[1]);
  

  //RECUPERATION DES DONNEES
  while (cin >> t1 >> s1 >> s2 >> t >> h >> ni) {
    if (!(h==1 && t==1) && t1<86400*4*7/lolo && t1>=86400*3*7/lolo) {
     if (t1>maxt) {maxt=t1;}
  ...
  cerr << atoi(argv[1]) << " " << average/number << " " << 0.5-aire << " " << sqrt(sv/number-((average/number)*(average/number))) << " " << shanon10 << " " << shanon5 << " " << shanon20 << " " << cre << endl;
  return 0;
}
